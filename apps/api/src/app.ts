...
export function createApp(io: Server) {
  ...
  app.post('/api/auth/register', async (req, res) => {
    ...
    if (!parsed.success) return res.status(400).json({ message: 'Sprawdź dane. Tag: 3–24 znaki, hasło: min. 8 znaków.' });
    const { tag, password, firstName, lastName } = parsed.data;
    if (db.prepare('SELECT 1 FROM users WHERE tag = ?').get(tag)) return res.status(409).json({ message: 'Ten tag jest już zajęty.' });
  ...
  app.post('/api/auth/login', async (req, res) => {
    ...
    if (!parsed.success) return res.status(400).json({ message: 'Nieprawidłowy tag lub hasło.' });
    const row = db.prepare('SELECT * FROM users WHERE tag = ?').get(parsed.data.tag) as Record<string, unknown> | undefined;
    if (!row || !await bcrypt.compare(parsed.data.password, String(row.password_hash))) return res.status(401).json({ message: 'Nieprawidłowy tag lub hasło.' });
    return res.json({ token: signToken(String(row.id)), user: mapUser(row) });
  });

  app.use('/api', requireAuth);
  app.get('/api/me', (_req, res) => {
    const row = db.prepare('SELECT * FROM users WHERE id = ?').get(res.locals.userId) as Record<string, unknown>;
  ...
  app.get('/api/users/search', (req, res) => {
    ...
    const rows = db.prepare(`SELECT * FROM users WHERE id != ? AND (tag LIKE ? OR first_name LIKE ? OR last_name LIKE ?) LIMIT 12`)
      .all(res.locals.userId, `%${query}%`, `%${query}%`, `%${query}%`) as Record<string, unknown>[];
  ...
  app.post('/api/conversations', (req, res) => {
    ...
    if (!parsed.success) return res.status(400).json({ message: 'Podaj poprawny tag.' });
    const other = db.prepare('SELECT id FROM users WHERE tag = ?').get(parsed.data.userTag) as { id: string } | undefined;
    if (!other || other.id === res.locals.userId) return res.status(404).json({ message: 'Nie znaleziono użytkownika.' });
    const existing = db.prepare(`SELECT cm.conversation_id id FROM conversation_members cm
      ...
      .get(res.locals.userId, other.id) as { id: string } | undefined;
    ...
    if (!existing) {
      ...
      db.transaction(() => {
        ...
        add.run(id, res.locals.userId, now); add.run(id, other.id, null);
      })();
    }
  ...
  app.patch('/api/conversations/:id', (req, res) => {
    if (!assertMember(req.params.id!, res.locals.userId)) return res.status(403).json({ message: 'Brak dostępu.' });
    const parsed = z.object({ accent: z.enum(accents as [string, ...string[]]) }).safeParse(req.body);
    if (!parsed.success) return res.status(400).json({ message: 'Nieprawidłowy kolor.' });
    db.prepare('UPDATE conversations SET accent = ? WHERE id = ?').run(parsed.data.accent, req.params.id);
    for (const userId of conversationMemberIds(req.params.id!)) io.to(`user:${userId}`).emit('conversation:new', getConversation(req.params.id!, userId));
    res.json(getConversation(req.params.id!, res.locals.userId));
  });
  app.get('/api/conversations/:id/messages', (req, res) => {
    const id = req.params.id!;
    if (!assertMember(id, res.locals.userId)) return res.status(403).json({ message: 'Brak dostępu.' });
    ...
    const rows = (query ? db.prepare(`SELECT id FROM messages WHERE conversation_id = ? AND text LIKE ? ORDER BY created_at ASC LIMIT 200`).all(id, `%${query}%`)
      : db.prepare('SELECT id FROM messages WHERE conversation_id = ? ORDER BY created_at ASC LIMIT 300').all(id)) as { id: string }[];
  ...
  app.post('/api/conversations/:id/messages', upload.single('file'), (req, res) => {
    ...
    if (!assertMember(conversationId, res.locals.userId)) return res.status(403).json({ message: 'Brak dostępu.' });
    const parsed = messageSchema.safeParse({ text: req.body.text ?? '' });
    if (!parsed.success || (!parsed.data.text && !req.file)) return res.status(400).json({ message: 'Wiadomość jest pusta.' });
    ...
    db.transaction(() => {
      ...
      if (req.file) db.prepare(`INSERT INTO attachments (id, message_id, kind, name, mime_type, size, storage_name)
        VALUES (?, ?, ?, ?, ?, ?, ?)`).run(randomUUID(), id, attachmentKind(req.file.mimetype), req.file.originalname,
          req.file.mimetype, req.file.size, req.file.filename);
      db.prepare('UPDATE conversations SET updated_at = ? WHERE id = ?').run(now, conversationId);
    })();
    const message = getMessage(id);
    for (const userId of conversationMemberIds(conversationId)) io.to(`user:${userId}`).emit('message:new', message);
    res.status(201).json(message);
  });
  app.post('/api/messages/:id/reactions', (req, res) => {
    const message = getMessage(req.params.id!);
    if (!assertMember(message.conversationId, res.locals.userId)) return res.status(403).json({ message: 'Brak dostępu.' });
    const parsed = z.object({ emoji: z.string().min(1).max(8) }).safeParse(req.body);
    if (!parsed.success) return res.status(400).json({ message: 'Nieprawidłowa reakcja.' });
    ...
    if (exists) db.prepare('DELETE FROM reactions WHERE message_id = ? AND user_id = ? AND emoji = ?').run(message.id, res.locals.userId, parsed.data.emoji);
    else db.prepare('INSERT INTO reactions (message_id, user_id, emoji) VALUES (?, ?, ?)').run(message.id, res.locals.userId, parsed.data.emoji);
    const updated = getMessage(message.id);
    for (const userId of conversationMemberIds(message.conversationId)) io.to(`user:${userId}`).emit('message:reaction', updated);
    res.json(updated);
  });
  app.use((error: unknown, _req: express.Request, res: express.Response, _next: express.NextFunction) => {
    console.error(error);
    if (error instanceof multer.MulterError) return res.status(400).json({ message: `Załącznik jest za duży (maks. ${config.MAX_FILE_SIZE_MB} MB).` });
