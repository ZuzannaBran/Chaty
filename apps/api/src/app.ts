...
export function createApp(io: Server) {
  ...
  app.post("/api/auth/register", async (req, res) => {
    ...
    db.prepare(
      `INSERT INTO users (id, tag, first_name, last_name, password_hash, avatar_color, created_at, public_key, encrypted_private_key, key_salt, key_iv)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    ).run(
      id,
      tag,
  ...
  app.post("/api/auth/login", async (req, res) => {
    ...
    const row = db
      ...
      .get(parsed.data.tag) as Record<string, unknown> | undefined;
    if (
      !row ||
    ...
    if (!row.public_key) {
      ...
      db.prepare(
        "UPDATE users SET public_key = ?, encrypted_private_key = ?, key_salt = ?, key_iv = ? WHERE id = ?",
      ).run(
        JSON.stringify(publicKey),
        encryptedPrivateKey,
        keySalt,
  ...
  app.get("/api/uploads/:storageName", (req, res) => {
    const row = db
      .prepare(
        `SELECT a.mime_type FROM attachments a JOIN messages m ON m.id = a.message_id
      JOIN conversation_members cm ON cm.conversation_id = m.conversation_id
      WHERE a.storage_name = ? AND cm.user_id = ?`,
      )
      .get(req.params.storageName, res.locals.userId) as
      { mime_type: string } | undefined;
    if (!row) return res.status(404).json({ message: "Nie znaleziono pliku." });
    res
      .type(row.mime_type)
  ...
  app.get("/api/me", (_req, res) => {
    const row = db
      ...
      .get(res.locals.userId) as Record<string, unknown>;
  ...
  app.get("/api/users/search", (req, res) => {
    ...
    const rows = db
      .prepare(
        `SELECT * FROM users WHERE id != ? AND (tag LIKE ? OR first_name LIKE ? OR last_name LIKE ?) LIMIT 12`,
      )
      .all(
        res.locals.userId,
        `%${query}%`,
  ...
  app.get("/api/conversations", (_req, res) =>
    res.json(listConversations(res.locals.userId)),
  );
  app.post("/api/conversations", (req, res) => {
    ...
    const other = db
      ...
      .get(parsed.data.userTag) as { id: string } | undefined;
    ...
    if (!existing) {
      ...
      db.transaction(() => {
        db.prepare(
          "INSERT INTO conversations (id, accent, created_at, updated_at) VALUES (?, ?, ?, ?)",
        ).run(id, accents[0], now, now);
  ...
  app.patch("/api/conversations/:id", (req, res) => {
    ...
    const parsed = z
      .object({ accent: z.enum(accents as [string, ...string[]]) })
      .safeParse(req.body);
    if (!parsed.success)
      return res.status(400).json({ message: "Nieprawidłowy kolor." });
    db.prepare("UPDATE conversations SET accent = ? WHERE id = ?").run(
      parsed.data.accent,
      req.params.id,
    );
    for (const userId of conversationMemberIds(req.params.id!))
      io.to(`user:${userId}`).emit(
        "conversation:new",
        getConversation(req.params.id!, userId),
      );
  ...
  app.get("/api/conversations/:id/messages", (req, res) => {
    ...
    const rows = (
      query
        ? db
            .prepare(
              `SELECT id FROM messages WHERE conversation_id = ? AND text LIKE ? ORDER BY created_at ASC LIMIT 200`,
            )
            .all(id, `%${query}%`)
        : db
            .prepare(
              "SELECT id FROM messages WHERE conversation_id = ? ORDER BY created_at ASC LIMIT 300",
            )
            .all(id)
    ) as { id: string }[];
  ...
  app.post(
    ...
    (req, res) => {
      ...
      db.transaction(() => {
        db.prepare(
          "INSERT INTO messages (id, conversation_id, sender_id, text, created_at, reply_to_message_id) VALUES (?, ?, ?, ?, ?, ?)",
        ).run(
          id,
          conversationId,
          res.locals.userId,
        ...
        if (req.file)
          db.prepare(
            `INSERT INTO attachments (id, message_id, kind, name, mime_type, size, storage_name, encryption_iv)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
          ).run(
            randomUUID(),
            id,
            req.body.originalMime
              ? attachmentKind(String(req.body.originalMime))
              : attachmentKind(req.file.mimetype),
            req.body.originalName || req.file.originalname,
            req.body.originalMime || req.file.mimetype,
            req.file.size,
            req.file.filename,
            req.body.encryptionIv || null,
          );
  ...
  app.post("/api/messages/:id/forward", (req, res) => {
    ...
    db.transaction(() => {
      db.prepare(
        `INSERT INTO messages (id, conversation_id, sender_id, text, created_at, forwarded_from_message_id)
        VALUES (?, ?, ?, ?, ?, ?)`,
      ).run(
        id,
  ...
  app.post("/api/messages/:id/reactions", (req, res) => {
    ...
    if (exists)
      ...
      ).run(message.id, res.locals.userId, parsed.data.emoji);
    else
      db.prepare(
