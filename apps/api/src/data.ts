...
export function mapUser(row: Row): User {
  return {
    ...
    publicKey: row.public_key
      ? (JSON.parse(String(row.public_key)) as JsonWebKey)
      : null,
  };
}
...
function getMessageReference(messageId: unknown) {
  if (!messageId) return null;
  const row = db
    .prepare(
      `SELECT m.id, m.sender_id, m.text, a.name attachment_name
